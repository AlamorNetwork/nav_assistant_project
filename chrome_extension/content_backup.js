...
function findSearchButton() {
    log('تلاش برای پیدا کردن دکمه جستجو...');
    ...
    for (const pattern of searchPatterns) {
        try {
            ...
            if (elements && elements.length > 0) {
                ...
                log(`پیدا شد با pattern "${pattern}": ${selector}`);
                return selector;
            }
        } catch (e) {
    ...
    // If no common patterns work, try to find any button with search-like text
    const allButtons = document.querySelectorAll('button, input[type="button"], input[type="submit"]');
    for (const btn of allButtons) {
        const text = (btn.innerText || btn.value || btn.title || '').trim();
        if (text.includes('جستجو') || text.toLowerCase().includes('search')) {
            const selector = generateUniqueSelector(btn);
            log(`پیدا شد از طریق متن: "${text}" -> ${selector}`);
            return selector;
        }
    }
    
    log('هیچ دکمه جستجویی پیدا نشد');
    return null;
}

function generateUniqueSelector(element) {
    // Generate the most specific selector for an element
    if (element.id) {
        return `#${element.id}`;
    }
    ...
    // Use value attribute for inputs
    if (element.tagName.toLowerCase() === 'input' && element.value) {
        return `input[value="${element.value}"]`;
    }
    
    // Use text content for buttons
    if (element.tagName.toLowerCase() === 'button' && element.innerText) {
        return `button:contains("${element.innerText.trim()}")`;
    }
...
async function checkSingleFund(fund) {
    ...
    try {
        ...
        log(`در حال دریافت تنظیمات از: ${API_BASE_URL}/configurations/${fund.name}`);
        ...
        log(`پاسخ سرور: Status ${response.status} ${response.statusText}`);
        
        if (!response.ok) {
            log(`تنظیمات صندوق ${fund.name} یافت نشد. Status: ${response.status}`, 'warn');
            
            // Try to use default config as fallback
            log('استفاده از کانفیگ پیش‌فرض...', 'info');
            config = {
                tolerance: 4.0,
                nav_search_button_selector: 'input[value="جستجو"]',
                expert_search_button_selector: 'input[value="جستجو"]',
                nav_price_selector: 'td.nav-price',
                total_units_selector: 'td.total-units',
                securities_list_selector: 'tbody tr',
                sellable_quantity_selector: 'td:nth-child(3)',
                expert_price_selector: 'td:nth-child(12)',
                date_selector: 'td.date',
                time_selector: 'td.time',
                increase_rows_selector: 'input[name="rows"]'
            };
            log('کانفیگ پیش‌فرض لود شد', 'success');
        } else {
        ...
        log(`جزئیات خطا: ${error.stack}`, 'error');
        
        // Use fallback config
        log('استفاده از کانفیگ fallback به دلیل خطا...', 'warn');
        config = {
            tolerance: 4.0,
            nav_page_url: 'https://krzetf5.irbroker.com/fund.do?method=navList&new_search=true',
            expert_price_page_url: 'https://krzetf5.irbroker.com/adjustedIp.do?new_search=true',
            nav_search_button_selector: 'input[type="submit"][value*="جستجو"], input[type="submit"], button:contains("جستجو")',
            expert_search_button_selector: 'input[type="submit"][value*="جستجو"], input[type="submit"], button:contains("جستجو")',
            nav_price_selector: '#fund_nav_table tbody tr:first-child td:nth-child(11)',
            total_units_selector: '#fund_nav_table tbody tr:first-child td:nth-child(9)',
        ...
        log('کانفیگ fallback با URL های صحیح لود شد', 'success');
    }
    ...
    if (!isOnNavPage && !isOnExpertPage) {
        ...
        // Auto-navigate to NAV page if not on correct page
        const navUrl = config.nav_page_url;
        if (navUrl) {
            log(`انتقال خودکار به صفحه NAV: ${navUrl}`, 'info');
            try {
                // Use chrome.tabs API to navigate to correct page
                await chrome.runtime.sendMessage({
                ...
                if (areUrlsMatching(newUrl, navUrl)) {
                    log(`انتقال موفق به صفحه NAV`, 'success');
                    // Retry the check after navigation
                    setTimeout(() => checkSingleFund(fund), 2000);
                    return;
                } else {
                    log(`انتقال ناموفق. URL فعلی: ${newUrl}`, 'warn');
                }
            } catch (error) {
                log(`خطا در انتقال به صفحه NAV: ${error.message}`, 'error');
            }
        }
        
        return; // Skip this fund if we can't navigate or we're not on its pages
    }
    ...
    if (selectedSecurityIndex === undefined && !(isOnExpertPage && (localState[`needsExpertData_${fund.name}`] || localState[`expertSuppressPrompt_${fund.name}`]))) {
        ...
        if (!localState[`listExpanded_${fund.name}`]) {
            ...
            // Check if expert search button selector is defined
            if (!config.expert_search_button_selector) {
                log(`expert_search_button_selector برای ${fund.name} تعریف نشده است. تلاش برای پیدا کردن خودکار...`, 'warn');
                
                // Try to auto-detect search button
                const autoSelector = findSearchButton();
                if (autoSelector) {
                    log(`expert search selector خودکار پیدا شد: ${autoSelector}`, 'success');
                    config.expert_search_button_selector = autoSelector;
                } else {
                    log(`نتوانستم دکمه جستجوی expert را به صورت خودکار پیدا کنم.`, 'error');
                }
            }
            
            const expertSearchButton = document.querySelector(config.expert_search_button_selector);
            if(expertSearchButton) {
                log(`دکمه جستجوی expert یافت شد، کلیک می‌کنم: ${config.expert_search_button_selector}`);
                await chrome.storage.local.set({ [`listExpanded_${fund.name}`]: true });
                
                // Try multiple click methods
                try {
                    expertSearchButton.click();
                    log(`کلیک اول expert انجام شد`);
                } catch (e) {
                    log(`خطا در کلیک اول expert: ${e.message}`, 'warn');
                }
                
                try {
                    expertSearchButton.dispatchEvent(new MouseEvent('click', { 
                        bubbles: true, 
                        cancelable: true, 
                        view: window 
                    }));
                    log(`کلیک دوم expert (MouseEvent) انجام شد`);
                } catch (e) {
                    log(`خطا در کلیک دوم expert: ${e.message}`, 'warn');
                }
                ...
                log(`دکمه جستجوی صفحه قیمت کارشناسی برای ${fund.name} با selector "${config.expert_search_button_selector}" یافت نشد.`, 'error');
    ...
    if (isOnNavPage) {
        ...
        if (!localState[`navSearchClicked_${fund.name}`]) {
            ...
            if (!config.nav_search_button_selector) {
                log(`nav_search_button_selector برای ${fund.name} تعریف نشده است. تلاش برای پیدا کردن خودکار...`, 'warn');
                
                // Try to auto-detect search button
                const autoSelector = findSearchButton();
                if (autoSelector) {
                    log(`selector خودکار پیدا شد: ${autoSelector}`, 'success');
                    config.nav_search_button_selector = autoSelector;
                } else {
                    log(`نتوانستم دکمه جستجو را به صورت خودکار پیدا کنم.`, 'error');
                    return;
                }
            }
...
async function shouldRunOnThisTab() {
    ...
    // Check if bot is active from storage
    const { botActive } = await chrome.storage.local.get('botActive');
    if (!botActive) {
        log("ربات غیرفعال است.", 'info');
        return false;
    }
    
    // If bot is active, run on ANY tab (remove bot-managed restriction)
    // This allows the bot to work on user-opened tabs too
    if (!isBotManagedTab) {
        log("این تب توسط کاربر باز شده است. ربات روی آن کار نمی‌کند.", 'info');
        
        // But if we're on a relevant page, mark this tab as bot-managed
        const currentUrl = window.location.href.toLowerCase();
        const isRelevantPage = currentUrl.includes('irbroker.com') || 
                              currentUrl.includes('fund.do') || 
                              currentUrl.includes('adjustedip') ||
                              currentUrl.includes('navlist');
        
        if (isRelevantPage) {
            log("صفحه مرتبط شناسایی شد. ربات فعال می‌شود.", 'success');
            
            // Add this tab to bot-managed tabs
            const stored = await chrome.storage.local.get('botManagedTabs');
            const botManagedTabs = stored.botManagedTabs || [];
            if (tabId && !botManagedTabs.includes(tabId)) {
                botManagedTabs.push(tabId);
    ...
    // Simple URL matching - avoid configuration fetch for speed
    const currentUrl = window.location.href.toLowerCase();
    const hasNavKeywords = currentUrl.includes('navlist') || currentUrl.includes('fund.do') || 
                          currentUrl.includes('nav') || currentUrl.includes('صافی') || 
                          currentUrl.includes('ارزش') || currentUrl.includes('irbroker');
    const hasExpertKeywords = currentUrl.includes('adjustedip') || currentUrl.includes('expert') || 
                             currentUrl.includes('price') || currentUrl.includes('کارشناسی') || 
                             currentUrl.includes('irbroker');
    
    log(`URL matching: ${currentUrl}`, 'info');
    
    if (hasNavKeywords || hasExpertKeywords) {
        log(`صفحه مرتبط شناسایی شد: ${hasNavKeywords ? 'NAV' : 'Expert'}`, 'success');
        return true;
    } else {
        log('صفحه مرتبط نیست', 'warn');
        return false;
    }
}
...
async function startMonitoring() {
    ...
    // Check if this is after a search button click (page reload)
    const stored = await new Promise(resolve => chrome.storage.local.get(['navSearchClicked', 'expertSearchClicked'], resolve));
    if (stored.navSearchClicked || stored.expertSearchClicked) {
        const pageType = stored.navSearchClicked ? 'NAV' : 'Expert';
        log(`صفحه ${pageType} بعد از کلیک جستجو reload شد - شروع تحلیل جدول...`, 'success');
        
        // Clear the flags
        await new Promise(resolve => chrome.storage.local.remove(['navSearchClicked', 'expertSearchClicked'], resolve));
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
	(async () => {
		try {
			...
			if (request.type === 'INITIALIZE_NAV_TAB') {
				log(`🎯 تب NAV پیکربندی شد برای صندوق: ${request.fundName}`, 'success');
				
				// Wait for page to fully load
				await sleep(2000);
				
				// Increase rows and click search on NAV page
				try {
					// Get configuration for this fund
					const { authToken } = await chrome.storage.sync.get('authToken');
					const headers = authToken ? { 'token': authToken } : {};
					const response = await fetch(`${API_BASE_URL}/configurations/${request.fundName}`, {
						headers: headers,
						method: 'GET',
						mode: 'cors'
					});
					...
					if (response.ok) {
						...
						// Use fallback config
						config = {
							nav_search_button_selector: 'input[type="submit"]',
							increase_rows_selector: 'input[name="rows"]'
						};
					}
					
					// Increase rows if selector exists
					const increaseRowsInput = document.querySelector(config.increase_rows_selector);
					if (increaseRowsInput) {
						increaseRowsInput.value = '1000';
						log('تعداد ردیف‌ها در NAV به 1000 افزایش یافت', 'info');
					}
					
					// Click search button
					let navSearchButton = document.querySelector(config.nav_search_button_selector);
					// Fallback selector for NAV search button
					if (!navSearchButton) {
						navSearchButton = document.querySelector('input[type="submit"][value*="جستجو"]') || 
									      document.querySelector('input[type="submit"]');
						if (navSearchButton) {
							log('استفاده از fallback selector برای دکمه جستجو NAV', 'info');
						}
					}
					if (navSearchButton) {
						log('کلیک روی دکمه جستجو در تب NAV', 'success');
						navSearchButton.click();
						
						// Alternative click methods
						try {
							navSearchButton.dispatchEvent(new MouseEvent('click', { 
								bubbles: true, 
								cancelable: true, 
								view: window 
							}));
						...
						// Form submit if in form
						try {
							const form = navSearchButton.closest('form');
							if (form && form.requestSubmit) {
								form.requestSubmit();
							}
						} catch (e) {}
						
						// Set flag to indicate search was clicked - wait for page reload
						await new Promise(resolve => chrome.storage.local.set({ navSearchClicked: true }, resolve));
						log('علامت کلیک جستجو NAV تنظیم شد - منتظار reload صفحه...', 'info');
					} else {
						log('دکمه جستجو در تب NAV یافت نشد', 'warn');
						
						// Show all buttons for debug
						const allButtons = document.querySelectorAll('input[type="submit"], input[type="button"], button');
						log(`دکمه‌های موجود در NAV (${allButtons.length}):`);
						allButtons.forEach((btn, i) => {
					...
					log(`خطا در پیکربندی تب NAV: ${error.message}`, 'error');
				}
			...
			if (request.type === 'INITIALIZE_EXPERT_TAB') {
				log(`🎯 تب Expert پیکربندی شد برای صندوق: ${request.fundName}`, 'success');
				
				// Wait for page to fully load
				await sleep(2000);
				
				// Increase rows and click search on Expert page
				try {
					// Get configuration for this fund
					const { authToken } = await chrome.storage.sync.get('authToken');
					const headers = authToken ? { 'token': authToken } : {};
					...
					if (response.ok) {
						...
						// Use fallback config
						config = {
							expert_search_button_selector: 'input[type="submit"]',
							increase_rows_selector: 'input[name="rows"]'
						};
					}
					...
					if (increaseRowsInput) {
						increaseRowsInput.value = '1000';
						log('تعداد ردیف‌ها در Expert به 1000 افزایش یافت', 'info');
					}
					
					// Click search button
					let expertSearchButton = document.querySelector(config.expert_search_button_selector);
					// Fallback selector for Expert search button
					if (!expertSearchButton) {
						expertSearchButton = document.querySelector('input[type="submit"][value*="جستجو"]') || 
										     document.querySelector('input[type="submit"]');
						if (expertSearchButton) {
							log('استفاده از fallback selector برای دکمه جستجو Expert', 'info');
						}
					}
					if (expertSearchButton) {
						log('کلیک روی دکمه جستجو در تب Expert', 'success');
						expertSearchButton.click();
						...
						// Set flag to indicate search was clicked - wait for page reload
						await new Promise(resolve => chrome.storage.local.set({ expertSearchClicked: true }, resolve));
						log('علامت کلیک جستجو Expert تنظیم شد - منتظار reload صفحه...', 'info');
					} else {
						log('دکمه جستجو در تب Expert یافت نشد', 'warn');
						
						// Show all buttons for debug
						const allButtons = document.querySelectorAll('input[type="submit"], input[type="button"], button');
						log(`دکمه‌های موجود در Expert (${allButtons.length}):`);
						allButtons.forEach((btn, i) => {
							const text = btn.value || btn.innerText || btn.textContent || '';
							log(`  ${i+1}. "${text}"`);
						});
					...
					log(`خطا در پیکربندی تب Expert: ${error.message}`, 'error');
				}
